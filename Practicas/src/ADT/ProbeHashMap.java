...
public class ProbeHashMap<K,V> extends AbstractHashMap<K,V> {
  ...
  protected V bucketPut(int h, K k, V v) {
    ...
    if (j >= 0)                               // this key has an existing entry
      return table[j].setValue(v); //lo sobreescribe
    table[-(j+1)] = new MapEntry<>(k, v);     // convert to proper index
    n++;
    return null; //si no había un valor antes
  }
  ...
  protected V bucketRemove(int h, K k) {
    int j = findSlot(h, k);
    if (j < 0) return null;                   // nothing to remove //signfica que no había un valor existente
