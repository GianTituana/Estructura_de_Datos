...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  public AbstractHashMap() { this(17); }                     // default capacity //con el this se invoca al segundo constructor y este a su vez con su otro this invoca al primer constructor y por eso ese es el primo por default
   ...
   */
  @Override
  public V get(K key) { return bucketGet(hashValue(key), key); } //hashValue te da la posición en la tabla
   ...
   */
  @Override
  public V put(K key, V value) { //porque a la final no uso el bucketPut cuando estoy en el main sino solo put
...
//  protected int hashValue(K key) { 
////	  return (3 * key.hashCode() + 5) % 11;	  
////  }
  /** Updates the size of the hash table and rehashes all entries. */
  private void resize(int newCap) {
    ArrayList<Entry<K,V>> buffer = new ArrayList<>(n); //del tamaño de todas las entradas
    for (Entry<K,V> e : entrySet()) //entrySet es la implementación que depende de las clases que heredan de esta
      buffer.add(e);
    capacity = newCap;
